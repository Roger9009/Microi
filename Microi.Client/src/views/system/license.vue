...
export default {
    ...
    methods: {
        async init() {
            ...
            const self = this;
            this.DiyCommon.Get("/api/License/GetConfig", function (result) {
        ...
        generateRegistrationFile() {
            ...
            self.DiyCommon.Post("/api/License/GenerateRegistrationFile", {
                Company:     self.applyForm.Company.trim(),
                Name:        self.applyForm.Name.trim(),
                Phone:       self.applyForm.Phone.trim(),
                IP:          "",
        ...
        loadLicenseList() {
            ...
            self.DiyCommon.Get("/api/License/List", { status: self.adminFilter || "" }, function (result) {
        ...
        adminApprove(row) {
            ...
            ElMessageBox.confirm(`确定审核通过 HID: ${row.HID} 的申请并签发 License？`, "审核确认", {
                ...
                self.DiyCommon.Post("/api/License/Approve", { HID: row.HID }, function (result) {
                    if (result && result.Code === 1) {
        ...
        adminReject(row) {
            ...
            ElMessageBox.prompt("请输入驳回原因", "驳回申请", { type: "warning", confirmButtonText: "驳回", cancelButtonText: "取消" })
                .then(({ value }) => {
                    self.DiyCommon.Post("/api/License/Reject", { HID: row.HID, RejectReason: value || "" }, function (result) {
                        if (result && result.Code === 1) {
        ...
        adminRevoke(row, revoke) {
            ...
            ElMessageBox.confirm(`确定${action} HID: ${row.HID} 的 License？`, `${action}确认`, {
                ...
                self.DiyCommon.Post("/api/License/Revoke", { HID: row.HID, Revoke: revoke }, function (result) {
                    if (result && result.Code === 1) {
        ...
        doIssue() {
            ...
            self.issuing = true;
            self.DiyCommon.Post("/api/License/Issue", {
        ...
        loadLogs() {
            ...
            self.DiyCommon.Get("/api/License/Logs", { hid: self.logHidFilter || "" }, function (result) {
        ...
        checkSuperAdmin() {
            const self = this;
            self.DiyCommon.Get("/api/License/List", { status: "", page: 1, pageSize: 1 }, function (result) {
